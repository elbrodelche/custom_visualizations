...
import * as LiquidFillGauge from './power_liquid_fill_gauge.js'
...
const vis: LiquidFillGaugeVisualization = {
  ...
  options: {
    ...
    waveFillWithGradient: {
      label: "Animate Waves",
      default: defaults.fillWithGradient,
      section: "Style",
      type: "boolean",
    },

    waveGradientFromColor: {
      label: "Gradient from Color",
      default: defaults.gradientFromColor,
      section: "Style",
      type: "string",
      display: "color",
    },
    waveGgradientToColor: {
  ...
  update(data, element, config, queryResponse, details) {
    ...
    const valueFormat = gaugeConfig.displayPercent
      ? null
      : datumField.value_format;
